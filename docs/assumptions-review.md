...
| 10 | Retail and Apparel covers 150-250 Moody's issuers | **measured and hand-curated** (notes/rating-history-file.md): after the manual scope pass, group dedupe and filing verification, 56 groups are in scope, actively rated and current filers (23 IG, 33 spec); 9 more usable historically only. A few hundred samples requires issuer-dates or added sectors | Was the basis for sample-size hopes; now a curated frame |
